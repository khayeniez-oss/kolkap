...
async function upsertContact(params: {
  ...
  if (error) {
    console.error("Failed to upsert Kolkap WhatsApp contact:", error);
    return null;
  }
...
function getErrorMessage(payload: unknown): string {
  ...
  const directMessage =
    ...
    cleanText(record.error_message) ||
    cleanText(record.errorDescription) ||
    cleanText(record.error_description) ||
  ...
  return safeJson(payload) || "Unknown error payload.";
}
...
function enrichRecipient(recipient: Record<string, unknown>) {
...
async function sendRecipientThroughTemplateApi(params: {
  ...
  const response = await fetch(sendUrl.toString(), {
    ...
    body: JSON.stringify({
      ...
      sendProvider: normalizeSendProvider(params.campaign.send_provider),
  ...
  if (ok) {
    await markRecipient({
      recipientId: cleanText(params.recipient.id),
      status: "sent",
    ...
    await markRecipient({
      recipientId: cleanText(params.recipient.id),
      status: "skipped",
      skipReason: result?.reason || "Skipped by template send API.",
      errorPayload: result || null,
    });
  } else {
    await markRecipient({
      recipientId: cleanText(params.recipient.id),
      ...
      errorPayload: result || {
        ...
        error: "Template send API failed.",
      },
    });
  }
...
export async function GET(req: Request) {
  ...
  if (campaignId) {
    ...
    if (campaignError) {
      console.error("Failed to load Kolkap WhatsApp campaign:", campaignError);
    ...
    if (includeRecipients) {
      ...
      recipients = ((data || []) as Record<string, unknown>[]).map(
        enrichRecipient
      );
    }
  ...
  if (error) {
    console.error("Failed to load Kolkap WhatsApp campaigns:", error);
...
export async function POST(req: Request) {
  ...
  try {
      ...
      return Response.json(
        {
          success: false,
          error:
            "Kolkap campaign sending currently supports Meta Cloud API only.",
        },
        { status: 400 }
      );
    }
    ...
    if (campaignError || !campaign?.id) {
      console.error("Failed to create Kolkap WhatsApp campaign:", campaignError);
    ...
    for (const recipient of parsed.recipients) {
      ...
      if (recipientError) {
        skippedCount += 1;
        console.error(
          "Failed to insert Kolkap WhatsApp campaign recipient:",
          recipientError
        );
      } else {
        insertedCount += 1;
    ...
    console.error("Create Kolkap WhatsApp campaign API error:", error);
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    if (action === "delete_campaign") {
      await supabaseAdmin
        .from("kolkap_whatsapp_template_send_logs")
        .delete()
        .eq("campaign_id", campaignId);
      ...
      if (recipientsDeleteError) {
        console.error(
          "Failed to delete Kolkap campaign recipients:",
      ...
      if (campaignDeleteError) {
        console.error("Failed to delete Kolkap campaign:", campaignDeleteError);
    ...
    for (const recipient of targetRecipients as Record<string, unknown>[]) {
      ...
      results.push({
        ...
        phoneE164: cleanText(recipient.phone_e164),
        ok: sendResult.ok,
    ...
    console.error("Send Kolkap WhatsApp campaign batch API error:", error);
